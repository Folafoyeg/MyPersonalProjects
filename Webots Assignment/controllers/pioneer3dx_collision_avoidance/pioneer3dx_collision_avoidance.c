...
// maximal speed motor of gripper
...
int main() {
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    if (timer == 50 && gripper_status == 0) {
      lift(0.00);
      gripper_status = 1;
    }
    if (timer == 100 && gripper_status == 1) {
      moveFingers(0.09);
      gripper_status = 2;
    }
    if (timer == 150 && gripper_status == 2) {
      moveFingers(0.01);
      gripper_status = 3;
    }
    if (timer == 200 && gripper_status == 3) {
      lift(0.05);
      gripper_status = 0;
      timer = 0;
    }
